...
function OAuthStrategy(options, verify) {
  ...
  //       the user profile.
  // This needs to be moved to the auth call and created during each authentication attempt.  This is allow for dynamic
  // consumer and consumer secrets to be called.
  ...
  //Add the parameters passed to oauth to the class
  ...
  //This value will be dynamically called.
...
OAuthStrategy.prototype.authenticate = function(req, options) {
  ...
  var meta = {
    ...
    consumerKey: req.oauth.clientId
  }

  var _oauth = new OAuth(this._requestTokenURL, this._accessTokenURL,
                          req.oauth.clientID,  req.oauth.clientSecret,
                          '1.0', null, this._signatureMethod,
                          null, this.customHeaders);
  ...
  if (req.query && req.query.oauth_token) {
    ...
    function loaded(err, oauthTokenSecret, state) {
      ...
      _oauth.getOAuthAccessToken(oauthToken, oauthTokenSecret, oauthVerifier, function(err, token, tokenSecret, params) {
        if (err) { return self.error(self._createOAuthError('Failed to obtain access token', err)); }
      
        function destroyed(err) {
          if (err) { return self.error(err); }
          if(params) {
              params._oauth = _oauth;
          } else {
              params = {};
              params._oauth = _oauth;
          }
    ...
    try {
      ...
      if (arity == 4) {
        this._requestTokenStore.get(req, oauthToken, meta, loaded);
          ...
          console.log("calling token store with " + oauthToken);
    ...
    var callbackURL = req.oauth.callbackURL || options.callbackURL || this._callbackURL;
    if (callbackURL) {
    ...
    _oauth.getOAuthRequestToken(params, function(err, token, tokenSecret, params) {
